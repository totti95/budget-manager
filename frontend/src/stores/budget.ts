...
export const useBudgetStore = defineStore('budget', () => {
  ...
  async function generateBudget(month: string) {
    ...
    try {
      ...
      error.value = err.response?.data?.message || 'Erreur lors de la génération du budget'
      throw err
    } finally {
      loading.value = false
    }
  ...
  async function updateBudget(id: number, data: { name: string }) {
    ...
    try {
      ...
      error.value = err.response?.data?.message || 'Erreur lors de la mise à jour du budget'
      throw err
    } finally {
      loading.value = false
    }
