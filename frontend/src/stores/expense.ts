...
export const useExpenseStore = defineStore('expense', () => {
  ...
  async function updateExpense(id: number, data: Partial<CreateExpenseData>) {
    ...
    try {
      ...
      error.value = err.response?.data?.message || 'Erreur lors de la mise à jour de la dépense'
      throw err
    } finally {
      loading.value = false
    }
  ...
  async function importCsv(budgetId: number, file: File) {
    ...
    try {
      ...
      error.value = err.response?.data?.message || 'Erreur lors de l\'import CSV'
      throw err
    } finally {
      loading.value = false
    }
  ...
  async function exportCsv(budgetId: number) {
    ...
    try {
      ...
      error.value = err.response?.data?.message || 'Erreur lors de l\'export CSV'
      throw err
    } finally {
      loading.value = false
    }
